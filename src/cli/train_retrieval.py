...
def train_retrieval(config_path: str) -> str:
    logging.basicConfig(level=logging.INFO, format="%(levelname)s %(message)s")
    config = _load_yaml(config_path)
    family = str(config.get("model_family", "retrieval"))
    ...
    run_id = f"{timestamp}_retrieval_e5"
    ...
    result = retr.fit_retrieval(train_df, test_df, config)
    y_train = train_df["topic_id"].astype(str)
    train_topic_counts = {str(t): int(c) for t, c in y_train.value_counts().items()}

    artifacts_dir = Path("artifacts") / "retrieval_e5" / run_id
    retr.save_retrieval(artifacts_dir, result)
    ad = str(artifacts_dir).replace("\\", "/")
    tr_sha = file_sha256(artifacts_dir / "embeddings_train.npy")
    te_sha = file_sha256(artifacts_dir / "embeddings_test.npy")

    k_list = (config.get("eval", {}) or {}).get("k_list", [1, 3, 5, 10])
    k = max(int(max(k_list)), 10)
    if result["embeddings_test"] is None:
        raise RuntimeError("expected embeddings_test for retrieval eval")
    y_pred, _ = retr.search_with_query_embeddings(
        result["index"],
        result["classes"],
        result["embeddings_test"],
        k,
    )
    _, _, latencies = retr.predict_topk(
    ...
    rmeta = {
        "encoder_name": str(result["encoder_name"]),
        "encoder_revision": str(result["encoder_revision"]),
        "normalize_embeddings": bool(result["normalize_embeddings"]),
        "faiss_metric": "IP",
        "embedding_dim": int(result["centroids"].shape[1]),
        "index_ntotal": int(result["index"].ntotal),
        "representation": str(config.get("retrieval", {}).get("representation", "centroid")),
        "classes_path": f"{ad}/classes.json",
        "e5_prefix_query": str(result["meta_file"]["e5_prefix_query"]),
        "e5_prefix_passage": str(result["meta_file"]["e5_prefix_passage"]),
        "encoder_batch_size": int(result["batch_size_used"]),
    ...
    artifacts_paths = {
        "themes_faiss": f"{ad}/themes.faiss",
        "classes": f"{ad}/classes.json",
        "meta": f"{ad}/meta.json",
        "embeddings_train": f"{ad}/embeddings_train.npy",
        "embeddings_test": f"{ad}/embeddings_test.npy",
    }
    ...
    metrics_payload = _build_metrics(
        ...
        retrieval_meta=rmeta,
        latency_device=str(result["device"]),
    )
...
def main(args: argparse.Namespace) -> int:
    run_id = train_retrieval(config_path=args.config)
    print(run_id)
    return 0
